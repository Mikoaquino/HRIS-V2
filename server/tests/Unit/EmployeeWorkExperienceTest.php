...
test('can simultaneously update existing and add multiple employee work experience', function () {
    ...
    $workExpCollection = $employee->workExperiences()->createMany($validated);

    $updateData['work_experiences'] = $workExpCollection->map(fn ($workExp) => [
        'id'                 => $workExp->id,
        'employee_id'        => $employee->id,
        'previous_employer'  => fake()->company(),
        'job_position'       => fake()->sentence(),
    ...
    $this->service->updateWorkExperiences($updateData);

    expect($employee->workExperiences->count())->toBe(7);

    foreach ($updateData['work_experiences'] as $workExp) {
        $this->assertDatabaseHas('employee_work_experiences', [
